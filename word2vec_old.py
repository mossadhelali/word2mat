...
#valid_window = 400  # Only pick dev samples in the head of the distribution.
valid_examples = validationWordIndices # np.random.choice(valid_window, valid_size, replace=False)
...
with tf.Session(graph=graph) as session:
  ...
  embeddings_to_store = embeddings.eval()
  
  timestr = str(datetime.datetime.now()).split('.')[0].replace(':', '_').replace(' ', '_')

  np.save('w2v_embeddings_' + timestr + '.npy', embeddings_to_store)

  print("stored embeddings in w2v_embeddings_" + str(embedding_size) + "_" + timestr + ".npy")
